...
# scope_note: AI-02 is partially delivered here.
#   This plan covers: scraping/HTML extraction sub-requirement of AI-02
#   (scrape public HTTPS URL, strip boilerplate via BeautifulSoup, cap at 32k chars).
#   The Redis caching sub-requirement of AI-02 ("caches raw scraped text in Redis for
#   1 hour so identical URLs skip re-scraping") is delivered in Phase 4 alongside
#   RATE-03 (Upstash Redis sliding window), where the Redis infrastructure is set up.
...
Scope note (AI-02): This plan delivers the scraping and HTML extraction portion of AI-02. The Redis URL caching portion of AI-02 ("caches raw scraped text in Redis for 1 hour so identical URLs skip re-scraping") is delivered in Phase 4 alongside RATE-03, where the Upstash Redis infrastructure is provisioned.
</objective>
...
```
  ...
  <done>
    ...
    - Redis URL caching is NOT implemented here — that is Phase 4 (RATE-03)
  </done>
</task>
...
Note: Redis URL caching (the second sub-requirement of AI-02) is NOT verified here — it is scoped to Phase 4 Plan alongside RATE-03.
</verification>
...
- AI-02 Redis caching sub-requirement is deferred to Phase 4 (RATE-03) — out of scope for this plan
</success_criteria>
