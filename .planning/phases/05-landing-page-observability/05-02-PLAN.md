...
must_haves:
  truths:
    ...
    - "graph_exported event helper is created in analytics.ts and will fire on downloads (stub for Phase 4 EXP-01/EXP-02 wiring — no export buttons exist in Phase 5 scope)"
  artifacts:
...
Output: PostHogProvider in layout, typed analytics.ts helpers, captureGraphGenerated event firing in app page handleSubmit success branch. captureGraphExported helper is created as a stub for Phase 4 (EXP-01/EXP-02) wiring — no export buttons exist in Phase 5 scope.
</objective>
...
**NOTE on captureGraphExported and captureGraphHistoryViewed:** These helpers are intentional stubs. No export buttons or history page exist in Phase 5 scope. They are created here so Phase 4 (EXP-01, EXP-02) and Phase 3 (FE-03) can import and call them without touching this file.
...
Keep all other page.tsx logic (state machine, AbortController, toast handling) completely unchanged.
  ...
  <verify>
    <automated>cd "/Users/uzi/Documents/Git Projects/instagraph-vc" && pnpm --filter web exec tsc --noEmit 2>&1 | head -20</automated>
    <manual>Check lib/analytics.ts exports captureGraphGenerated, captureGraphExported, captureGraphHistoryViewed. Check captureGraphExported and captureGraphHistoryViewed have stub comments explaining Phase 4/3 wiring. Check app/app/page.tsx imports captureGraphGenerated and calls it in the success branch after setGraph(data.graph).</manual>
  </verify>
  <done>TypeScript compiles clean. analytics.ts exports all three typed helpers. captureGraphExported and captureGraphHistoryViewed are documented stubs with no callers in Phase 5 (per OBS-02 scope — graph_exported wiring is a Phase 4 EXP-01/EXP-02 deliverable, graph_history_viewed is a Phase 3 FE-03 deliverable). app/app/page.tsx calls captureGraphGenerated in the handleSubmit success branch with node_count, edge_count, and source_type arguments.</done>
</task>
...
5. Verify captureGraphExported and captureGraphHistoryViewed have STUB comments noting their Phase 4/3 callers
6. Verify app/app/page.tsx calls captureGraphGenerated in success branch
</verification>
...
- captureGraphExported is a Phase 5 stub — no callers exist yet. graph_exported event will fire in Phase 4 when EXP-01 (JSON export) and EXP-02 (PNG export) buttons are built and wired to this helper
- captureGraphHistoryViewed is a Phase 5 stub — no callers exist yet. graph_history_viewed will fire in Phase 3 when FE-03 (history page) is built
